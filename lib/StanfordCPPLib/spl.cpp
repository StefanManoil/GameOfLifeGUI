...
GWindow* GEvent::getWindow() const {
    return static_cast<GWindow*>(_source);
}
...
GWindow* GCanvas::getWindow() const {
    return _window;
}
...
void GCanvas::setWindow(GWindow *window) {
    _window = window;
}
...
GWindow* GInteractor::getWindow() const {
    return nullptr;
}
...
void GWindow::ensureForwardTarget() {
    if (!_canvas) {
        ...
        GThread::runOnQtGuiThread([this]() {
            ...
            _canvas->setWindow(this); // make sure the created canvas associates with the window
...
LifeDisplay* GWindow::getDisplay() const {
    return _display;
}
...
void GWindow::setDisplay(LifeDisplay* display) {
    _display = display;
}
